...
#: 읽는 순서. 뒤에 오는 파일이 앞의 값을 덮어쓰지 않는다(먼저 정의된 값 우선).
ENV_FILES: tuple[Path, ...] = (
    PROJECT_ROOT / ".env",
    SRC_ROOT / "rootkeepers" / "collectors" / ".env",
)


def load_env() -> list[Path]:
    """존재하는 `.env`를 전부 읽는다. 어떤 진입점에서 불러도 결과가 같다.

    python-dotenv가 없어도(선택 의존성) 조용히 넘어간다 — 환경변수를 직접
    넣어 쓰는 Docker 실행에서는 dotenv가 필요 없기 때문이다.

    Returns:
        실제로 읽은 파일 목록. 하나도 없으면 빈 리스트.
    """
    try:
...
__all__ = [
    ...
    "ENV_FILES", "load_env", "project_dir",
]
