...
평소에는 대시보드만 쓴다 — 터미널 CLI는 HTTP로 `/api/ingest`에 보내고 기록은
콘솔이 맡는다(reporting.py). 그래서 dashboard/ 안에 둔다.

예외가 하나 있다. **콘솔이 꺼져 있으면** CLI가 이 모듈을 직접 임포트해서 쓴다
(reporting._store_locally). 그러지 않으면 콘솔이 없는 동안의 터미널 설치가
통째로 유실된다. 즉 이 DB는 프로세스 두 개가 쓸 수 있다 — 트랜잭션을 짧게
유지해야 하는 이유다(SQLite의 파일 잠금에 의존한다).
